...
export interface MinMaxAvgDaysChartProps extends MultiProps {
  stats: AdminCarResearchStatistics;
  theme: StyledComponentTheme;
}

class MinMaxAvgDaysChart extends React.Component<MinMaxAvgDaysChartProps> {
  chart = null;
  ...
  createChart() {
    ...
    const chart = am4core.create('minMaxAvgDaysChart', am4charts.XYChart);

    chart.data = [
      {
        category: translations.admin.app,
        min: lowestTimeOnMarketApp,
        max: highestTimeOnMarketApp,
        avg: averageTimeOnMarketApp,
      },
      {
        category: translations.admin.api,
        min: lowestTimeOnMarketAPI,
        max: highestTimeOnMarketAPI,
        avg: averageTimeOnMarketAPI,
      },
    ];
  ...
  public render() {
    return (
      <div id="minMaxAvgDaysChart" style={{ width: '100%', height: '400px' }} />
    );
  }
}

export default multi(withTheme(MinMaxAvgDaysChart));
